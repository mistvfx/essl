...
<_Toast@Label>:
    ...
    canvas:
        ...
        RoundedRectangle:
            size: self.size
            pos: self.pos
            #pos: int(self.center_x - self.texture_size[0]), int(self.center_y - self.texture_size[1] / 2.)
        Color:
...
def toast(text, bg_color, length_long=False):
    _Toast(text=text, bg=bg_color).show(length_long)
